              ...
              <div className="space-y-2">
                ...
                <div className="w-full px-4 py-3 rounded-xl border border-slate-200 focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-500 transition-all bg-white relative">
                   <input 
                      ...
                      onChange={async (e) => {
                        ...
                        for (const file of files) {
                          try {
                            const fileKey = await import('../../../services/serverlessService').then(m => m.uploadEvidenceToS3(file));
                            setDocuments(prev => [...prev, fileKey]);
                          } catch (err) {
                            console.error("S3 Upload Failed", err);
                            alert("Failed to upload " + file.name + " to S3.");
                          }
                        }
                      }}
                   ...
                   <div className="flex items-center justify-center gap-2 text-slate-500">
                      ...
                      <span className="text-sm font-medium">Click to upload files directly to S3</span>
                   </div>
                </div>
                {documents.length > 0 && (
                    ...
                    {documents.map((doc, index) => (
                      <div key={index} className="relative group aspect-square rounded-xl border border-slate-200 overflow-hidden bg-slate-50 flex items-center justify-center">
                        <div className="flex flex-col items-center justify-center text-slate-400 p-2 text-center">
                          ...
                          <span className="text-[10px] font-semibold mt-1 break-all">{doc.substring(doc.lastIndexOf('/') + 1)}</span>
                        </div>
                        <button 
                          type="button"
                          onClick={(e) => {
